...
- [x] **Pre-TestFlight security check** — done 2026-07-01, found and fixed 2 real issues: (1) `App.tsx` bootstrap's catch block could silently skip the consent wall entirely if `ensureAnonSession()` threw — now fails closed (`setShowConsent(true)`); (2) `increment_analyze_meal_usage` RPC had default `PUBLIC`/`anon`/`authenticated` EXECUTE grants despite being `SECURITY DEFINER` with an unchecked `p_user_id` param — any client could grief another user's daily quota directly; revoked, now `service_role`-only. Both verified live. See CHANGES.md for full detail.
